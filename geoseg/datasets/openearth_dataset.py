...
class OpenEarthDataset(Dataset):
    def __init__(self, data_root='data/openearth/test', mode='val', img_dir='images_1024', mask_dir='masks_1024',
    ...
    def load_img_and_mask(self, index):
        ...
        img = Image.fromarray(self.load_multiband(img_name)).convert('RGB')
        mask = Image.fromarray(self.load_grayscale(mask_name)).convert('L')
        return img, mask
    ...
    def show_img_mask_seg(seg_path, img_path, mask_path, start_seg_index):
        ...
        for i in range(len(seg_list)):
            ...
            img_seg = load_grayscale(f'{seg_path}/{seg_id}')
            ...
            mask = load_grayscale(f'{mask_path}/{seg_id}')
            ...
            img = load_multiband(f'{img_path}/{img_id}')
    ...
    def show_seg(seg_path, img_path, start_seg_index):
        ...
        for i in range(len(seg_list)):
            seg_id = seg_list[i]
            # img_seg = cv2.imread(f'{seg_path}/{seg_id}', cv2.IMREAD_UNCHANGED)
            img_seg = load_grayscale(f'{seg_path}/{seg_id}', cv2.IMREAD_UNCHANGED)
            img_seg = img_seg.astype(np.uint8)
            img_seg = Image.fromarray(img_seg).convert('P')
            img_seg.putpalette(np.array(PALETTE, dtype=np.uint8))
            img_seg = np.array(img_seg.convert('RGB'))
            img_id = str(seg_id.split('.')[0])+'.tif'
            # img = cv2.imread(f'{img_path}/{img_id}', cv2.IMREAD_COLOR)
            img = load_multiband(f'{img_path}/{img_id}', cv2.IMREAD_COLOR)
            # img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            ax[i, 0].set_axis_off()
            ax[i, 0].imshow(img)
            ax[i, 0].set_title('RS IMAGE '+img_id)
